...
def define_hparam_space(learning_rate=None):
    """
    Define the hyperparameter search space.
    
    Args:
        learning_rate (float, optional): Single learning rate value from command line
...
def grid_search_hyperparams(learning_rate=None):
    """
    ...
    Args:
        learning_rate (float, optional): Single learning rate value from command line
    """
    # Get hyperparameter search space with optional learning rate
    hparam_space = define_hparam_space(learning_rate)
...
if __name__ == "__main__":
    ...
    if use_wandb_sweep:
        # Run WandB sweep
        sweep_id = run_wandb_sweep(project_name=train_config["project_name"], count=6)
        print(f"WandB sweep completed with ID: {sweep_id}")
    else:
        # Run TensorBoard grid search with command line learning rate if provided
        best_hparams, best_loss = grid_search_hyperparams(args.learning_rate)

...
def visualize_layer_outputs(model, image, device, save_path=None, fig_size=(15, 5)):
    ...
    # Get the number of layers in the model.features
    num_layers = len(model.features)
    
    # Create feature extractor
    # Use layer 2 for block 1 features (after first ReLU)
    # Use layer (num_layers-2) for block 2 features (after last ReLU)
    extractor = FeatureExtractor(model, [1, num_layers-2])
    ...
    block1_features = features[0][0]  # [C, H, W]
    ...
    axes[1].set_title(f'Block 1 Output ({block1_features.shape[1]}x{block1_features.shape[2]})')
    axes[1].axis('off')
    
    # Plot Block 2 features (8x8)
    block2_features = features[1][0]  # [C, H, W]
    # Select the first 16 feature maps (or fewer if there are less)
    ...
    axes[2].set_title(f'Block 2 Output ({block2_features.shape[1]}x{block2_features.shape[2]})')
    axes[2].axis('off')
...
def visualize_model_layers(model, dataloader, num_images=1, device = torch.device("cuda" if torch.cuda.is_available() else "cpu"), save_dir=None):
    ...
    if len(labels.shape) > 1 and labels.shape[1] > 1:
        # If the labels are one-hot encoded, get the class index
        label = torch.argmax(labels[:num_images], dim=1).item()
    else:
        # If the labels are already class indices
